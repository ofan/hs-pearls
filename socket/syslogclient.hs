...
while :: (String -> Bool) -> (String -> IO ()) -> IO ()
while p action = do
  i <- getLine
  when (p i) $ action i >> while p action

main = do
  args <- getArgs
  print $ null args
  if null args then fail "Not enough argument." else do
    let port = head args
    h <- openlog "127.0.0.1" port "testprog"
    putStrLn $ "Connected to port " ++ port ++ "..."
    putStrLn "Enter message('quit' to quit):"
    while (/= "quit") (syslog h USER INFO)
    closelog h
    putStrLn "Bye."
